 ...
 */
@Named(value = "contactApplicationForCustomer")
@ApplicationScoped
public class ContactApplicationForCustomer {
...
//	add some data from db to app
	public ContactApplicationForCustomer() {
...
// when loading, and after adding or deleting, the customer contact list need to be refreshed 	
	public void updateCustomerContactList() {
		...
		if (customerContacts!= null &&customerContacts.size()>0) {
			...
			for (CustomerContact customerContact: customerContactManagedBean.getContactsByStaff()) {
	...
	public void searchAllCustomerContact() {
		customerContacts.clear();
		for (CustomerContact customerContact : customerContactManagedBean.getContactsByCustomer()) {
			customerContacts.add(customerContact);
